...
async function resolveDir(segments: string[], dir: string): Promise<string | null> {
...
async function handler(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
    ...
    if (dir) {
        for (const name of [`${req.method}.json`, 'index.json']) {
            try {
                const { _status: status = 200, _delay = 0, ...body } = JSON.parse(
    ...
    return Response.json({ error: 'mock not found', path: req.url }, { status: 404 })
}
